...
public class JdbcSessionMappingStorage implements SessionMappingStorage {
  ...
  public void clean() {
    String sql =
        "DELETE FROM cas_client_session"
            + " WHERE session_id NOT IN (SELECT session_id FROM spring_session)";
