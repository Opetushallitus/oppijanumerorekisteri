...
export const fetchL10n = () => async (dispatch: any) => {
    ...
    try {
        const henkiloUiLocalisations = await http.get(
            urls.url("henkilo-ui.l10n"),
        )
        dispatch(receiveLocalisations(henkiloUiLocalisations))

        const localisationPalveluLocalisations = await http.get(
            urls().url("lokalisointi.localisation", {category: "henkilo-ui"}),
        )
        const localisationsByLocale = mapLocalisationsByLocale(
            localisationPalveluLocalisations,
        )
