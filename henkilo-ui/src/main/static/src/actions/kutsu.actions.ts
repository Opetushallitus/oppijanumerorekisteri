...
export const fetchKutsus = (payload, offset, amount) => async (
    dispatch,
    getState,
) => {
        ...
        dispatch(
            addGlobalNotification({
                ...
                title: localizeWithState(
                    "KUTSUTUT_VIRKAILIJA_FETCHING_FAILED",
                    getState(),
                ),
            }),
        )
...
export const clearKutsuList = () => dispatch =>
    dispatch({type: CLEAR_KUTSU_LIST})
...
export const createHenkiloByToken = (temporaryToken, payload) => (
    dispatch,
    getState,
) => {
