...
const henkiloLinkitysRequest = (oidHenkilo) => ({type: FETCH_HENKILO_LINKITYKSET_REQUEST, oidHenkilo});
const henkiloLinkitysSuccess = (oidHenkilo, linkityksetByOid) => ({type: FETCH_HENKILO_LINKITYKSET_SUCCESS, linkityksetByOid, oidHenkilo});
const henkiloLinkitysFailure = (oidHenkilo) => ({type: FETCH_HENKILO_LINKITYKSET_FAILURE, oidHenkilo});
/**
 * Hakee henkilön varmentaja linkitykset käyttöoikeuspalvelusta. Ei yritä hakea uudestaan jos tiedot on jo haettu.
 * @param oidHenkilo
 * @returns {Function}
 */
export const fetchHenkiloLinkitykset = (oidHenkilo: string) => async (dispatch: Dispatch, getState: HenkiloLinkitysGetState) => {
