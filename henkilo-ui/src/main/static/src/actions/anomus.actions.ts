...
const receiveHaetutKayttooikeusryhmatFailure = nimi => ({
...
export const fetchHaetutKayttooikeusryhmat = (
    ...
    try {
        const haetutKayttooikeusryhmat: Array<HaettuKayttooikeusryhma> = await http.get(
            url,
        )
        await dispatch(
            fetchOrganisations(
        ...
        dispatch(
            receiveHaetutKayttooikeusryhmatSuccess(haetutKayttooikeusryhmat),
        )
    } catch (error) {
