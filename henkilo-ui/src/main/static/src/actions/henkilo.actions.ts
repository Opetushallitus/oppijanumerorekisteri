                ...
                errorMessages.forEach(errorMessage =>
                    dispatch(
                        addGlobalNotification(
                            createUpdateHenkiloErrorNotification(errorMessage),
                        ),
                    ),
                )
            } else {
...
const createUpdateHenkiloErrorNotification = (
    title: string,
...
const getUpdateHenkiloErrorMessages = (error, L): Array<string> => {
    ...
    if (
        error.status === 400 &&
        error.message &&
        error.message.indexOf("invalid.hetu") !== -1
    ) {
        errorMessages.push(L["NOTIFICATION_HENKILOTIEDOT_TALLENNUS_VIRHE_HETU"])
    }
    if (
        error.status === 400 &&
        JSON.stringify(error).includes("socialsecuritynr.already.exists")
    ) {
        errorMessages.push(
...
export const fetchHenkiloOrgs = oidHenkilo => (dispatch, getState) => {
    ...
    return http.get(url).then(json => {
        dispatch(
            fetchOrganisations(
                json.map(orgHenkilo => orgHenkilo.organisaatioOid),
            ),
        ).then(organisationsAction =>
            dispatch(
                receiveHenkiloOrgsSuccess(json, getState().organisaatio.cached),
            ),
        )
    })
}
...
export const passivoiHenkiloOrg = (oidHenkilo, oidHenkiloOrg) => dispatch => {
    ...
    return http
        ...
        .catch(() =>
            dispatch(errorPassivoiHenkiloOrg(oidHenkilo, oidHenkiloOrg)),
        )
}
...
export const fetchHenkiloDuplicates = oidHenkilo => async (
    dispatch,
    getState,
) => {
    ...
    try {
        ...
        if (duplicates.length === 0) {
            dispatch(
                addGlobalNotification({
                    ...
                    title: localizeWithState(
                        "NOTIFICATION_DUPLIKAATIT_TYHJA_LISTA",
                        getState(),
                    ),
                    autoClose: 10000,
                }),
        ...
        let errorMessage =
            localizeWithState("NOTIFICATION_DUPLIKAATIT_VIRHE", getState()) +
            " " +
            oidHenkilo
        if (
            ...
            errorMessage =
                localizeWithState(
                    "NOTIFICATION_DUPLIKAATIT_HAKEMUKSET_ATARU_VIRHE",
                    getState(),
                ) +
                " " +
                oidHenkilo
            ...
            error.message.startsWith(
                "Failed fetching hakemuksetDto for henkilos",
            )
        ) {
            errorMessage =
                localizeWithState(
                    "NOTIFICATION_DUPLIKAATIT_HAKEMUKSET_HAKUAPP_VIRHE",
                    getState(),
                ) +
                " " +
                oidHenkilo
...
export const fetchHenkiloHakemukset = oid => async (dispatch, getState) => {
    ...
    try {
        ...
        let errorMessage =
            localizeWithState(
                "NOTIFICATION_HENKILO_HAKEMUKSET_VIRHE",
                getState(),
            ) +
            " " +
            oid
        if (
            ...
            errorMessage =
                localizeWithState(
                    "NOTIFICATION_HENKILO_HAKEMUKSET_ATARU_VIRHE",
                    getState(),
                ) +
                " " +
                oid
            ...
            errorMessage =
                localizeWithState(
                    "NOTIFICATION_HENKILO_HAKEMUKSET_HAKUAPP_VIRHE",
                    getState(),
                ) +
                " " +
                oid
