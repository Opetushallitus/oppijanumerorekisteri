...
export const organisaatioToOrganisaatioSelectObject = (
    ...
    return organisaatioHierarkiaToOrganisaatioSelectObject(
        [organisaatio],
        locale,
    )
}
        ...
        if (organisaatio.children) {
            const parentNames =
                organisaatio.oid === PropertySingleton.state.rootOrganisaatioOid
                    ? []
                    : [
                          ...organisaatioSelectObject.parentNames,
                          organisaatioSelectObject.name,
                      ]
            mapOrganisaatioLevelsRecursively(
...
const isRyhma = (organisaatio: OrganisaatioSelectObject): boolean =>
    ...
    organisaatio.organisaatiotyypit.includes("Ryhma")
...
export const organisaatioHierarchyRoots = (
    ...
    // First sort by name:
    orgs = R.sortBy(
        (org: OrganisaatioHenkilo) =>
            toLocalizedText(locale, org.organisaatio.nimi),
        orgs,
    )
    const byOid = {}
    ...
    R.forEach(org => {
        if (org.parentOid) {
            ...
            if (parent) {
                ...
                if (
                    R.findIndex(R.pathEq(["oid"], org.oid))(parent.children) < 0
                ) {
                    parent.children.push(org)
                    parent.children = R.sortBy(
                        (org: OrganisaatioWithChildren) =>
                            toLocalizedText(locale, org.nimi),
                    )(parent.children)
                }
            } else {
...
export const getOrganisationsOrRyhmas = (
    ...
    return isRyhma
        ? organisaatios.filter(
              organisaatio => organisaatio.tyypit.indexOf("Ryhma") !== -1,
          )
        : organisaatios.filter(
              organisaatio => organisaatio.tyypit.indexOf("Ryhma") === -1,
          )
