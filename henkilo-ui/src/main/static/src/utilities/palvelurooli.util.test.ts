...
describe('Test organisaatioutilities', () => {
    ...
    it('hasPalveluRooliByOrganisaatioOid should return correct result', () => {
        ...
        const organisaatiot = [organisaatio1, organisaatio2];
        expect(
            hasPalveluRooliByOrganisaatioOid(organisaatiot, '123', 'OPPIJANUMEROREKISTERI_MANUAALINEN_YKSILOINTI')
        ).toBeTruthy();
        expect(hasPalveluRooliByOrganisaatioOid(organisaatiot, '234', 'KAYTTOOIKEUDET_TESTIOIKEUS')).toBeTruthy();
        expect(
            hasPalveluRooliByOrganisaatioOid(organisaatiot, 'nosuchid', 'OPPIJANUMEROREKISTERI_MANUAALINEN_YKSILOINTI')
        ).toBeFalsy();
    ...
    it('organisaatioContainsAnyPalveluRooli should return correct result', () => {
        ...
        expect(
            organisaatioContainsAnyPalveluRooli(organisaatio, ['FOO', 'OPPIJANUMEROREKISTERI_MANUAALINEN_YKSILOINTI'])
        ).toBeTruthy();
    });
