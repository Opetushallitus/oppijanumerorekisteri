import {
    hasAnyPalveluRooli,
    hasPalveluRooliByOrganisaatioOid,
    kayttooikeusMatchesAnyPalveluRooli,
...
describe("Test organisaatioutilities", () => {
    const kayttooikeus1 = {
        palvelu: "OPPIJANUMEROREKISTERI",
        oikeus: "DUPLIKAATTINAKYMA",
    }
    const kayttooikeus2 = {
        palvelu: "OPPIJANUMEROREKISTERI",
        oikeus: "MANUAALINEN_YKSILOINTI",
    }
    const kayttooikeus3 = {palvelu: "KAYTTOOIKEUDET", oikeus: "TESTIOIKEUS"}
    const palveluRoles1 = [
        "FOO_ROLE",
        "OPPIJANUMEROREKISTERI_DUPLIKAATTINAKYMA",
    ]
    ...
    it("parsePalveluRoolit should parse correct set of palveluroolit", () => {
        ...
        const organisaatio2: KayttooikeusOrganisaatiot = {
            organisaatioOid: "234",
            kayttooikeudet: [kayttooikeus3],
        }
    ...
    it("should return correct result for hasAnyPalveluRooli", () => {
        ...
        expect(
            hasAnyPalveluRooli([organisaatio1, organisaatio2], palveluRoles1),
        ).toBeTruthy()
    })
    ...
    it("organisaatioContainsAnyPalveluRooli should return correct result", () => {
        ...
        expect(
            organisaatioContainsAnyPalveluRooli(organisaatio, ["FOO", "VIRHE"]),
        ).toBeFalsy()
        expect(
            organisaatioContainsAnyPalveluRooli(organisaatio, [
                "FOO",
                "OPPIJANUMEROREKISTERI_MANUAALINEN_YKSILOINTI",
            ]),
        ).toBeTruthy()
    ...
    it("should return correct result for kayttooikeusMatchesAnyPalveluRooli", () => {
        expect(
            kayttooikeusMatchesAnyPalveluRooli(kayttooikeus1, palveluRoles1),
        ).toBeTruthy()
        expect(
            kayttooikeusMatchesAnyPalveluRooli(kayttooikeus1, palveluRoles2),
        ).toBeFalsy()
        expect(
            kayttooikeusMatchesAnyPalveluRooli(kayttooikeus2, palveluRoles1),
        ).toBeFalsy()
    })
})