...
const FORMATS = [
    {
        ...
        isValid: localizableText =>
            Array.isArray(localizableText) && localizableText.length > 0,
        getValue: (localizableText: any, uiLang: Locale) => {
            const value: any = R.find(R.propEq("kieli", uiLang.toUpperCase()))(
                localizableText,
            )
            return value ? value.nimi : value
        },
        getFallbackValue: localizableText =>
            typeof localizableText[0] === "object" &&
            localizableText[0] !== null
    ...
    },
    {
        ...
        isValid: localizableText =>
            Array.isArray(localizableText.texts) &&
            localizableText.texts.length > 0,
        getValue: (localizableText: TextGroup, uiLang: Locale) => {
            const value = R.find(R.propEq("lang", uiLang.toUpperCase()))(
                localizableText.texts,
            )
    ...
    },
    {
        ...
        isValid: localizableText =>
            typeof localizableText === "object" && localizableText !== null,
        getValue: (localizableText, uiLang: Locale) =>
            localizableText[uiLang.toLowerCase()],
        getFallbackValue: localizableText =>
            localizableText[Object.keys(localizableText)[0]],
    },
]
...
export function toLocalizedText(
    ...
    return R.pipe(
        ...
        R.map(format =>
            getValue(format, localizableText, uiLang, fallbackValue),
        ),
        R.find(hasValue),
    )(FORMATS)
}