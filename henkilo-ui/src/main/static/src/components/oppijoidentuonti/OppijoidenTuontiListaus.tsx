...
class OppijoidenTuontiListaus extends React.Component<Props> {
        ...
        const columns = [
            {
                ...
                accessor: henkilo =>
                    this.renderYksilointiTilaIcon(henkilo.yksilointiTila),
                id: "tila",
                className: "yksilointi-tila-sarake",
            },
            {
                Header: this.renderSortableHeader(
                    "OPPIJOIDEN_TUONTI_LUONTIAIKA",
                    "CREATED",
                ),
            ...
            },
            {
                Header: this.renderHeader(
                    "OPPIJOIDEN_TUONTI_HENKILOTUNNUS_SYNTYMAIKA",
                ),
            ...
            {
                Header: this.renderSortableHeader(
                    "OPPIJOIDEN_TUONTI_NIMI",
                    "NAME",
                ),
                accessor: (henkilo: OppijaList) =>
                    this.renderOppijaLinkki(henkilo),
                id: "nimi",
            },
            {
                Header: this.renderHeader("OPPIJOIDEN_TUONTI_VIRHEET"),
                accessor: (henkilo: OppijaList) =>
                    this.renderYksilointiTilaText(henkilo.yksilointiTila),
                id: "virheet",
            },
        ]
        return (
    ...
    renderSortableHeader(localizationKey: string, headerKey: string) {
        ...
        return (
            <span
                style={{display: "block"}}
                onClick={() =>
                    this.props.onChangeSorting(headerKey, newSortDirection)
                }
                className="oph-bold sortable-header"
            >
                {this.props.L[localizationKey]}{" "}
                {this.props.sortKey === headerKey ? (
                    this.renderSortIcon()
                ) : (
    ...
    renderHetuTaiSyntymaaika(henkilo: OppijaList) {
        return (
            henkilo.hetu ||
            (henkilo.syntymaaika
                ? moment(henkilo.syntymaaika).format("l")
                : null)
        )
    }

    renderYksilointiTilaText(arvo: string) {
        return YKSILOINTI_TILAT[arvo]
            ? YKSILOINTI_TILAT[arvo](this.props.L)
            : ""
    }
