...
type OrganisaatioSelectProps = {
    ...
    onClose?: (
        arg0: React.SyntheticEvent<HTMLButtonElement> | null | undefined,
    ) => void
}
...
export class OrganisaatioSelect extends React.Component<
    OrganisaatioSelectProps,
    State
> {
    constructor(props: OrganisaatioSelectProps) {
        super(props)
        const allOrganisaatiot = this._sortAlphabetically(
    ...
    render() {
        return (
            <div className="organisaatio-select">
                <p className="oph-h3">
                    {this.props.L["OMATTIEDOT_ORGANISAATIO_VALINTA"]}
                </p>
                <input
                    className="oph-input"
    ...
    _renderRow = (renderParams: any) => {
        const organisaatio: OrganisaatioSelectObject = this.state.organisaatiot[
            renderParams.index
        ]
        return (
            <div
    ...
    _renderOrganisaatioNimi = (organisaatio: OrganisaatioSelectObject) => {
        return (
            <div className="organisaatio-nimi">
                {organisaatio.name}{" "}
                {this._renderOrganisaatioTyypit(organisaatio)}
            </div>
        )
    }

    _renderOrganisaatioTyypit = (organisaatio: OrganisaatioSelectObject) => {
        return organisaatio.organisaatiotyypit &&
            organisaatio.organisaatiotyypit.length > 0
            ? `(${organisaatio.organisaatiotyypit.toString()})`
            : null
    }

    _renderParents = (
        organisaatio: OrganisaatioSelectObject,
    ): React.ReactNode => {
    ...
    _renderSuunniteltuNote = (organisaatio: OrganisaatioSelectObject) =>
        organisaatio.status === "SUUNNITELTU" ? (
            <div className="suunniteltu">
                {this.props.L["ORGANISAATIONVALINTA_SUUNNITELTU"]}
            </div>
        ) : null

    _renderPassiivinenNote = (organisaatio: OrganisaatioSelectObject) =>
        organisaatio.status === "PASSIIVINEN" ? (
            <div className="passiivinen">
                {this.props.L["ORGANISAATIONVALINTA_PASSIIVINEN"]}
            </div>
        ) : null
    ...
    onFilter(event: React.SyntheticEvent<HTMLInputElement>) {
        ...
        if (previousSearchWord.length > currentSearchWord.length) {
            if (
                this.state.allOrganisaatiot.length !==
                this.state.organisaatiot.length
            ) {
                const organisaatiot = this._filterAndSortOrganisaatios(
                    this.state.allOrganisaatiot,
                    currentSearchWord,
                )
    ...
    _filterAndSortOrganisaatios(
        ...
        const notStartingWithSearchWord = (
            organisaatio: OrganisaatioSelectObject,
        ) =>
            !organisaatio.name
                .toLowerCase()
        ...
        const organisaatioFilteredBySearchword = this._organisaatiotFilteredBy(
            organisaatiot,
            containsSearchword,
        )
        ...
        return [
            ...organisaatiotStartingWithSearchwordSortedByParentName,
            ...organisaatiotNotStartingWithSearchword,
        ]
    }
    ...
    _sortOrganisaatiotByParentName(
        ...
        const organisaatiotHavingParents: any = this._organisaatiotFilteredBy(
            organisaatiot,
            hasParent,
        )
        const organisaatiotNotHavingParents = this._organisaatiotFilteredBy(
            organisaatiot,
            noParent,
        )
        ...
        return [
            ...
            ...R.sortBy(R.compose(R.toLower, R.last, R.prop("parentNames")))(
