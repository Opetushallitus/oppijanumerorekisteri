...
} from "../../../../actions/omattiedot.actions"
...
class UserContentContainer extends React.Component<Props, State> {
    constructor(props: Props) {
        ...
        const henkiloUpdate = props.henkilo.henkilo
            ? JSON.parse(JSON.stringify(props.henkilo.henkilo))
            : {}
        henkiloUpdate.anomusilmoitus =
            props.omattiedot && props.omattiedot.anomusilmoitus
    ...
    componentWillReceiveProps(nextProps: Props) {
        ...
        henkiloUpdate.anomusilmoitus =
            nextProps.omattiedot && nextProps.omattiedot.anomusilmoitus
    ...
    render() {
        const kayttajaTyyppi =
            ...
            this.props.henkilo.kayttaja.kayttajaTyyppi
            ...
            <div className="henkiloViewUserContentWrapper">
                <div className="header">
                    <p className="oph-h2 oph-bold">
                        {this.props.L["HENKILO_PERUSTIEDOT_OTSIKKO"] +
                    ...
                    <ul>
                        {this._validKutsumanimi() ? null : (
                            <li>
                                {
                                    this.props.L[
                                        "NOTIFICATION_HENKILOTIEDOT_KUTSUMANIMI_VIRHE"
                                    ]
                                }
                            </li>
                        )}
                ...
                >
                    <ul>
                        {this._validYksilointi() ? null : (
                            <li>{this._getYksilointivirhe()}</li>
                        )}
                    </ul>
                </LocalNotification>
            </div>
    ...
    _discard() {
        const henkiloUpdate = JSON.parse(
            JSON.stringify(this.props.henkilo.henkilo),
        ) // deep copy
        henkiloUpdate.anomusilmoitus =
            this.props.omattiedot && this.props.omattiedot.anomusilmoitus
        this.setState({
    ...
    _update() {
        ...
        if (henkiloUpdate.kayttajanimi !== undefined) {
            this.props.updateAndRefetchKayttajatieto(
                henkiloUpdate.oidHenkilo,
                henkiloUpdate.kayttajanimi,
            )
        }
    ...
    async anomus(henkiloUpdate: any) {
        if (this.props.view === "OMATTIEDOT" && this.props.omattiedot.isAdmin) {
            const initialAnomusilmoitusValue = this.props.omattiedot
                .anomusilmoitus
            const url = urls.url(
    ...
    _updateModelField(event: any) {
        this.setState({
            henkiloUpdate: StaticUtils.updateFieldByDotAnnotation(
                this.state.henkiloUpdate,
                event,
            ),
        })
    ...
    _validYksilointi() {
        return !(
            this.props.henkilo.henkilo.yksilointivirheet &&
            this.props.henkilo.henkilo.yksilointivirheet.length
        )
    }
    ...
    _getYksilointivirhe() {
        ...
        const yksilointivirheMap = {
            ...
            MUU_UUDELLEENYRITETTAVA:
                "HENKILO_YKSILOINTIVIRHE_MUU_UUDELLEENYRITETTAVA",
            MUU: "HENKILO_YKSILOINTIVIRHE_MUU",
        }
        const virheKey =
            yksilointivirheMap[virhe.yksilointivirheTila] ||
            "HENKILO_YKSILOINTIVIRHE_OLETUS"
        return virhe.uudelleenyritysAikaleima
            ? `${this.props.L[virheKey]} ${
                  this.props.L["HENKILO_YKSILOINTIVIRHE_UUDELLEENYRITYS"]
              } ${moment(virhe.uudelleenyritysAikaleima).format(
                  PropertySingleton.getState().PVM_DATE_TIME_FORMAATTI,
              )}`
            : this.props.L[virheKey]
    }
}
