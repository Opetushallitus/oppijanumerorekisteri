...
class HenkiloViewOpenKayttooikeusanomus extends React.Component<Props, State> {
    ...
    constructor(props: Props) {
        ...
        this.tableHeadings = this.headingList.map(heading => ({
            ...heading,
            label: this.L[
                heading.localizationKey ? heading.localizationKey : heading.key
            ],
        }))
    ...
    _getKayttooikeusAnomukset = (props: Props): Array<any> => {
        return props.kayttooikeus && props.kayttooikeus.kayttooikeusAnomus
    ...
    createRows() {
        ...
        this._rows = this._getKayttooikeusAnomukset(this.props).map(
            (
                ...
                [headingList[0]]: moment(
                    haettuKayttooikeusRyhma.anomus.anottuPvm,
                ),
                [headingList[1]]:
                ...
                [headingList[4]]: this.createSelitePopupButton(
                    haettuKayttooikeusRyhma.anomus.perustelut,
                ),
                ...
                [headingList[7]]: (
                    <DatePicker
                        ...
                        filterDate={date =>
                            date.isBefore(moment().add(1, "years"))
                        }
                    />
                ),
                [headingList[8]]: this.L[
                    haettuKayttooikeusRyhma.anomus.anomusTyyppi
                ],
                [headingList[9]]: this.props.isOmattiedot
                    ? this.anomusHandlingButtonsForOmattiedot(
                          haettuKayttooikeusRyhma,
                          idx,
                      )
                    : this.anomusHandlingButtonsForHenkilo(
                          haettuKayttooikeusRyhma,
                          idx,
                      ),
            }),
    ...
    anomusHandlingButtonsForOmattiedot(
        haettuKayttooikeusRyhma: HaettuKayttooikeusryhma,
        idx: number,
    ) {
        return (
    ...
    anomusHandlingButtonsForHenkilo(
        haettuKayttooikeusRyhma: HaettuKayttooikeusryhma,
        idx: number,
    ) {
        ...
        return (
            <div>
                <div
                    className="anomuslistaus-myonnabutton"
                    style={{display: "table-cell", paddingRight: "10px"}}
                >
                    <MyonnaButton
                        myonnaAction={() =>
                    ...
                    <PopupButton
                        ...
                        popupTitle={
                            <span className="oph-h3 oph-strong">
                                {
                                    this.L[
                                        "HENKILO_KAYTTOOIKEUSANOMUS_HYLKAA_HAKEMUS"
                                    ]
                                }
                            </span>
                        }
                        ...
                        popupButtonClasses={
                            "oph-button oph-button-cancel oph-button-small"
                        }
                        popupStyle={{
                            right: "0px",
                            width: "20rem",
                        ...
                        disabled={
                            noPermission ||
                            this.state.disabledHylkaaButtons[
                                haettuKayttooikeusRyhma.id
                            ]
                        }
                        popupContent={
    ...
    updateHaettuKayttooikeusryhma = (
        ...
        const alkupvm: string = dates.alkupvm.format(
            PropertySingleton.state.PVM_DBFORMAATTI,
        )
        const loppupvm: string = dates.loppupvm.format(
            PropertySingleton.state.PVM_DBFORMAATTI,
        )
        if (this.props.updateHaettuKayttooikeusryhma) {
            this.props.updateHaettuKayttooikeusryhma(
                id,
                tila,
                alkupvm,
                loppupvm,
    ...
    hasNoPermission(organisaatioOid: string, kayttooikeusryhmaId: number) {
        return (
            ...
            !(
                this.props.kayttooikeus.grantableKayttooikeus[
                    organisaatioOid
                ] &&
                this.props.kayttooikeus.grantableKayttooikeus[
                    organisaatioOid
                ].includes(kayttooikeusryhmaId)
            )
        )
    }
    ...
    _getAnojaOidByRowIndex = (index: number): string => {
        return this.props.kayttooikeus.kayttooikeusAnomus[index].anomus.henkilo
            .oid
    }
    ...
    _parseAnojaKayttooikeusryhmat(anojaOid: string): void {
        ...
        http.get(url)
            .then(
                (
                    myonnettyKayttooikeusryhmat: Array<
                    ...
                    const kayttooikeudet: Array<KayttooikeusryhmaData> = myonnettyKayttooikeusryhmat
                        .filter(
                            (
                                ...
                                myonnettyKayttooikeusryhmat.tila !==
                                KAYTTOOIKEUDENTILA.ANOTTU,
                        )
                    ...
                    const kayttooikeusRyhmatByAnoja = this.state
                        .kayttooikeusRyhmatByAnoja
            ...
            .catch((error: any) => {
                ...
                console.error(
                    `Anojan ${anojaOid} käyttöoikeuksien hakeminen epäonnistui`,
                )
            })
    }

    _parseAnojaKayttooikeus = (
        myonnettyKayttooikeusryhma: MyonnettyKayttooikeusryhma,
    ): KayttooikeusryhmaData => {
        const kayttooikeusryhmaNimiTexts =
            myonnettyKayttooikeusryhma.ryhmaNames &&
            myonnettyKayttooikeusryhma.ryhmaNames.texts
        const kayttooikeusryhmaNimi = kayttooikeusryhmaNimiTexts
            ? localizeTextGroup(
                  kayttooikeusryhmaNimiTexts,
                  this.props.locale,
              ) || ""
            : ""
        const organisaatioNimi = this._parseOrganisaatioNimi(
            myonnettyKayttooikeusryhma,
        )
        return {
    ...
    _parseOrganisaatioNimi = (
        myonnettyKayttooikeusryhma: MyonnettyKayttooikeusryhma,
    ): string => {
        const organisaatio = this.props.organisaatioCache[
            myonnettyKayttooikeusryhma.organisaatioOid
        ]
        return organisaatio && organisaatio.nimi
            ? organisaatio.nimi[this.props.locale] ||
                  ...
                  organisaatio.oid
            : localize(
                  "HENKILO_AVOIMET_KAYTTOOIKEUDET_ORGANISAATIOTA_EI_LOYDY",
                  this.props.l10n,
                  this.props.locale,
              )
    }
    ...
    _parseVoimassaPvm = (
        ...
        const noLoppupvm = localize(
            "HENKILO_AVOIMET_KAYTTOOIKEUDET_EI_LOPPUPVM",
            this.props.l10n,
            this.props.locale,
        )
        if (!myonnettyKayttooikeusryhma.voimassaPvm) {
            ...
            myonnettyKayttooikeusryhma.tila !== KAYTTOOIKEUDENTILA.SULJETTU
        ) {
            return myonnettyKayttooikeusryhma.voimassaPvm
                ? moment(
                      new Date(myonnettyKayttooikeusryhma.voimassaPvm),
                  ).format()
                : noLoppupvm
        }
    ...
    _hasAnojaKayttooikeusData = (anojaOid: string): boolean => {
        return this.state.kayttooikeusRyhmatByAnoja.some(
            (anojaKayttooikeusryhmaData: AnojaKayttooikeusryhmaData) =>
                anojaKayttooikeusryhmaData.anojaOid === anojaOid,
        )
    }

    _findAnojaKayttooikeusData = (
        anojaOid: string,
    ): AnojaKayttooikeusryhmaData | null | undefined => {
        return R.find(R.propEq("anojaOid", anojaOid))(
            this.state.kayttooikeusRyhmatByAnoja,
        )
    }
                ...
                <div>
                    {!this.props.piilotaOtsikko && (
                        <div className="header">
                            <p className="oph-h2 oph-bold">
                                {
                                    this.L[
                                        "HENKILO_AVOIMET_KAYTTOOIKEUDET_OTSIKKO"
                                    ]
                                }
                            </p>
                        </div>
                    ...
                    <div>
                        <Table
                            ...
                            data={this._rows}
                            noDataText={
                                this.L["HENKILO_KAYTTOOIKEUS_AVOIN_TYHJA"]
                            }
                            ...
                            subComponent={
                                this.props.isAnomusView
                                    ? this.fetchKayttooikeusryhmatByAnoja.bind(
                                          this,
                                      )
                                    : undefined
                            }
