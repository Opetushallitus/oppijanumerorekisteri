...
type Props = OwnProps & {
    ...
    updateHenkiloAndRefetch: (
        arg0: Henkilo,
        arg1: GlobalNotificationConfig | null | undefined,
    ) => void
...
class HenkiloViewContactContent extends React.Component<Props, State> {
    ...
    createContent() {
        const isEmail = (label: string) =>
            label === PropertySingleton.state.SAHKOPOSTI

        const content: Array<React.ReactNode> = this.state.contactInfo
            .filter(
                yhteystiedotRyhmaFlat =>
                    this.state.yhteystietoRemoveList.indexOf(
                        yhteystiedotRyhmaFlat.id,
                    ) === -1,
            )
            .filter(
                yhteystiedotRyhmaFlat =>
                    this.state.yhteystietoRemoveList.indexOf(
                        yhteystiedotRyhmaFlat.henkiloUiId,
                    ) === -1,
            )
            .map((yhteystiedotRyhmaFlat, idx) => (
                <div key={idx}>
                    ...
                    {yhteystiedotRyhmaFlat.value.map(
                        (yhteystietoFlat, idx2) => (
                                ...
                                yhteystietoFlat.value ? (
                                    <Columns columns={2} className="labelValue">
                                        <span className="oph-bold">
                                            {
                                                this.props.L[
                                                    yhteystietoFlat.label
                                                ]
                                            }
                                        </span>
                                        <Field
                                            inputValue={
                                                yhteystietoFlat.inputValue
                                            }
                                            changeAction={this._updateModelField.bind(
                                                this,
                                                yhteystietoFlat,
                                            )}
                                            isEmail={isEmail(
        ...
        if (!this.state.readOnly) {
            content.push(
                <div
                    ...
                    onClick={() =>
                        this._createYhteystiedotRyhma(
                            PropertySingleton.getState().TYOOSOITE,
                        )
                    }
                    key="add-new"
                >
                ...
                <div>
                    <div className="header">
                        <p className="oph-h2 oph-bold">
                            {this.props.L["HENKILO_YHTEYSTIEDOT_OTSIKKO"]}
                        </p>
                    </div>
                    {this.props.henkilo.henkilo.turvakielto ? (
                        <div className="oph-h3 oph-bold midHeader">
                            {this.props.L["YHTEYSTIETO_TURVAKIELTO"]}
                        </div>
                    ) : null}
                ...
                {this.state.readOnly ? (
                    ...
                    <div className="henkiloViewEditButtons">
                        <EditButtons
                            ...
                            isValidForm={
                                this.state.modified &&
                                this.state.contactInfoErrorFields.length === 0
                            }
                        />
                    </div>
                )}
        ...
        this.state.yhteystietoRemoveList.forEach(yhteystietoId =>
            this.henkiloUpdate.yhteystiedotRyhma.splice(
                this.henkiloUpdate.yhteystiedotRyhma.findIndex(
                    yhteystieto =>
                        yhteystieto.id === yhteystietoId ||
                        yhteystieto.henkiloUiId === yhteystietoId,
                ),
                1,
            ),
        )
    ...
    _updateModelField(
        contactInfo: any,
        event: ReactSelectOption & React.SyntheticEvent<HTMLInputElement>,
    ) {
    ...
    _initialiseYhteystiedot = (
        ...
        henkiloUpdate.yhteystiedotRyhma.map((yhteystiedotRyhma, idx) => {
            ...
            yhteystiedotRyhma.yhteystieto = yhteystietoFlatList.value.map(
                yhteystietoFlat => ({
    ...
    createFlatYhteystieto(
        ...
        return {
            value: contactInfoTemplate.map((template, idx2) => ({
                ...
                value:
                    yhteystietoList.filter(
                        yhteystieto =>
                            yhteystieto.yhteystietoTyyppi === template.label,
                    )[0] &&
                    yhteystietoList.filter(
                        yhteystieto =>
                            yhteystieto.yhteystietoTyyppi === template.label,
            ...
            name:
                ...
                yhteystietotyypit.filter(
                    kieli => kieli.value === yhteystiedotRyhma.ryhmaKuvaus,
                )[0][locale],
