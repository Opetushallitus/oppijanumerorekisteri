...
export class EmailVerificationPage extends React.Component<Props, State> {
    constructor(props: Props) {
        ...
        this.state = {
            validForm: props.henkilo.yhteystiedotRyhma
                ? validateYhteystiedotRyhmaEmails(
                      props.henkilo.yhteystiedotRyhma,
                  )
                : false,
            henkilo: props.henkilo,
            emailFieldCount: notEmptyYhteystiedotRyhmaEmailCount(
                props.henkilo.yhteystiedotRyhma,
            ),
        }
    }
    ...
    componentDidMount() {
        ...
        if (this.state.emailFieldCount === 0) {
            const yhteystiedotRyhma = clone(
                this.state.henkilo.yhteystiedotRyhma,
            )
            ...
            if (
                ...
                this.state.henkilo.yhteystiedotRyhma[0].yhteystieto.length === 0
            ) {
        ...
        return (
            <div className="infoPageWrapper" id="email-verification-page">
                <h2 className="oph-h2 oph-bold">
                    {this.props.L["SAHKOPOSTI_VARMENNUS_OTSIKKO"]}
                </h2>
                ...
                <div style={{textAlign: "center"}}>
                    <Button
                        action={this.verifyEmailAddresses.bind(this)}
                        isButton
    ...
    async verifyEmailAddresses() {
        try {
            ...
            const loginTokenValidationCode = await http.get(
                loginTokenValidationCodeUrl,
            )
            if (loginTokenValidationCode !== "TOKEN_OK") {
                ...
                const redirectParams = await http.post(
    ...
    emailChangeEvent(
        yhteystiedotRyhmaIndex: number,
        yhteystietoIndex: number,
        value: string,
    ): void {
        let yhteystiedotRyhma: Array<YhteystietoRyhma> = this.state.henkilo
            .yhteystiedotRyhma
        yhteystiedotRyhma[yhteystiedotRyhmaIndex].yhteystieto[
            yhteystietoIndex
    ...
    onEmailRemove(
        ...
        const emailNotOnlyYhteystietoInYhteystietoryhma = yhteystiedotRyhma[
            yhteystiedotRyhmaIndex
        ].yhteystieto
            .filter(
                (yhteystieto: Yhteystieto) =>
                    yhteystieto.yhteystietoTyyppi !==
                    PropertySingleton.state.SAHKOPOSTI,
            )
            .some(
                (yhteystieto: Yhteystieto) =>
        ...
        if (emailNotOnlyYhteystietoInYhteystietoryhma) {
            ...
            yhteystietoRyhma.yhteystieto = remove(
                yhteystietoIndex,
                1,
                yhteystietoRyhma.yhteystieto,
            )
        } else {
            yhteystiedotRyhma = remove(
                yhteystiedotRyhmaIndex,
                1,
                yhteystiedotRyhma,
            )
