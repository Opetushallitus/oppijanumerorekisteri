...
type Props = {
    ...
    isAdmin: boolean
    henkilohakuAction: (
        arg0: HenkilohakuCriteria,
        arg1: HenkilohakuQueryparameters,
    ) => void
...
class HenkilohakuPage extends React.Component<Props, State> {
    ...
    componentWillReceiveProps(nextProps: Props): void {
        const newState = {
            showNoDataMessage:
                !nextProps.henkilohakuResult.length &&
    ...
    render() {
        return (
            <div className="wrapper">
                <div className="oph-h2 oph-bold henkilohaku-main-header">
                    {this.L["HENKILOHAKU_OTSIKKO"]}
                </div>
                <DelayedSearchInput
                    setSearchQueryAction={this.updateToSearchModel(
                        "nameQuery",
                    ).bind(this)}
                ...
                />
                <HenkilohakuFilters
                    noOrganisationAction={this.updateToSearchModel(
                        "noOrganisation",
                        true,
                    ).bind(this)}
                    suborganisationAction={this.updateToSearchModel(
                        "subOrganisation",
                        true,
                    ).bind(this)}
                    duplikaatitAction={this.updateToSearchModel(
                        "dublicates",
                        true,
                    ).bind(this)}
                    passiivisetAction={this.updateToSearchModel(
                        "passivoitu",
                        true,
                    ).bind(this)}
                    ...
                    organisaatioSelectAction={this.selectOrganisaaOid.bind(
                        this,
                    )}
                    clearOrganisaatioSelection={this.clearOrganisaatio.bind(
                        this,
                    )}
                    selectedRyhma={this.state.ryhmaOid}
                    ryhmaSelectionAction={this.selectRyhmaOid.bind(this)}
                    selectedKayttooikeus={
                        this.state.henkilohakuModel.kayttooikeusryhmaId
                    }
                    kayttooikeusSelectionAction={this.updateToSearchModel(
                        "kayttooikeusryhmaId",
                    ).bind(this)}
                />
                <div className="oph-h3 oph-bold henkilohaku-result-header">
                    ...
                    this.props.henkilohakuResultCount === 0
    ...
    createRows(headingKeys: Array<string>): Array<any> {
        return this.props.henkilohakuResult.map(henkilo => ({
            [headingKeys[0]]: (
                <Link to={`/virkailija/${henkilo.oidHenkilo}`}>
                    {henkilo.nimi || ""}
                </Link>
            ),
    ...
    selectRyhmaOid(ryhmaOption: any) {
        ...
        this.setState(
            {ryhmaOid: ryhmaOption.value, henkilohakuModel},
            this.searchQuery,
        )
    }
    ...
    updateToSearchModel(key: string, isEvent?: boolean) {
        return (entity: any) => {
            ...
            const callback =
                key === "nameQuery" && entity.value.length === 1
                    ? undefined
                    : this.searchQuery
    ...
    searchQuery(shouldNotClear?: boolean): void {
        ...
        const henkilohakuModel = {...this.state.henkilohakuModel}
        if (
            henkilohakuModel.nameQuery ||
            henkilohakuModel.organisaatioOids ||
            henkilohakuModel.kayttooikeusryhmaId
        ) {
