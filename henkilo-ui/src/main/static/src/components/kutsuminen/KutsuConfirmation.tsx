...
export default class KutsuConfirmation extends React.Component<Props, State> {
    ...
    render() {
        ...
        return (
            <Modal
                ...
                onClose={this.props.modalCloseFn}
            ...
            >
                <div className="confirmation-modal">
                    <i
                        ...
                        aria-hidden="true"
                    />
                    <span className="oph-h1 oph-strong">
                        {L["VIRKAILIJAN_LISAYS_ESIKATSELU_OTSIKKO"]}
                    </span>
                    <p>
                        {L["VIRKAILIJAN_LISAYS_ESIKATSELU_TEKSTI"]}{" "}
                        {this.props.basicInfo.email}
                    </p>
                    <span className="oph-h2 oph-strong">
                        {L["VIRKAILIJAN_LISAYS_ESIKATSELU_ALAOTSIKKO"]}
                    </span>
                    {this.props.addedOrgs.map(this.renderAddedOrg.bind(this))}
                    <div className="row">
                        {this.state.sent ? (
                            <Button action={this.onClose.bind(this)}>
                                {L["VIRKAILIJAN_LISAYS_LAHETETTY"]}
                            </Button>
                        ) : (
                            <Button
                                action={this._sendInvitation.bind(this)}
    ...
    renderAddedOrg(org: any) {
        return (
            <div key={org.organisation.oid}>
                ...
                {org.selectedPermissions.map(
                    this.renderAddedOrgPermission.bind(this),
                )}
            </div>
        )
    }
    ...
    async sendInvitation(
        ...
        const sahkoposti =
            this.props.basicInfo.email && this.props.basicInfo.email.trim()
        const payload = {
            ...
            saate: this.props.basicInfo.saate
                ? this.props.basicInfo.saate
                : undefined,
