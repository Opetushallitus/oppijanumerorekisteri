...
export const AnojaKayttooikeusryhmat = (props: Props) => {
    const data: AnojaKayttooikeusryhmaData | null | undefined = path(
        ["data"],
        props,
    )
    const error: boolean | null | undefined = path(["data", "error"], props)
    const kayttooikeudet:
        | Array<KayttooikeusryhmaData>
        | null
        | undefined = path(["data", "kayttooikeudet"], props)
    const headings = [
        {
            Header:
                props.l10n[props.locale][
                    "HENKILO_KAYTTOOIKEUSANOMUS_KAYTTOOIKEUSRYHMAT_MAIN_HEADER"
                ],
            columns: [
                {
                    Header:
    ...
    if (data === undefined) {
        ...
        return (
            <LocalNotification
                ...
                title={localize(
                    "NOTIFICATION_ANOMUKSEN_ANOJAN_OIKEUKSIEN_HAKU_VIRHE",
                    props.l10n,
                    props.locale,
                )}
                toggle={true}
    ...
    return (
        <LocalNotification
            ...
            title={localize(
                "NOTIFICATION_ANOMUKSEN_ANOJAN_KAYTTOOIKEUDET_TYHJA",
                props.l10n,
                props.locale,
            )}
            toggle={true}
