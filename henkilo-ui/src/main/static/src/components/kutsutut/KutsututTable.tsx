...
type OwnProps = {
    ...
    isLoading: boolean
    allFetched: boolean
    fetchKutsus: (arg0: any, arg1: boolean | null | undefined) => void
...
class KutsututTable extends React.Component<Props, State> {
    ...
    render() {
        ...
        const data = this.props.kutsus.map(kutsu => ({
            ...
            KUTSUTUT_KUTSU_LAHETETTY_OTSIKKO: this.createKutsuLahetettyCell(
                kutsu,
            ),
        ...
        return (
            <div className="kutsututTableWrapper oph-table">
                <Table
                    ...
                    fetchMoreSettings={{
                        isActive:
                            !this.props.allFetched && !this.props.isLoading,
                        fetchMoreAction: this.onSubmitWithoutClear.bind(this),
                    }}
                    tableLoading={this.props.isLoading}
                    subComponent={(row: any) => (
                        <KutsuDetails
                            kutsu={this.props.kutsus.find(
                                kutsu => kutsu.id === row.original.id,
                            )}
                            L={this.props.L}
            ...
            <div>
                {kutsu.organisaatiot.map(org => (
                    <div key={org.organisaatioOid}>
                        {toLocalizedText(this.props.locale, org.nimi) ||
                            org.organisaatioOid}
                    </div>
                ))}
            </div>
        ...
        return (
            <span>
                ...
                {sent.add(1, "months").isBefore(moment()) ? (
                    <span className="oph-red">
                        {this.props.L["KUTSUTUT_VIRKAILIJAT_KUTSU_VANHENTUNUT"]}
                    </span>
                ) : null}
            </span>
        )
        ...
        if (newSort) {
            this.setState(
                ...
                () => {
                    if (
                        !stateSort ||
                        newSort.id !== stateSort.id ||
                        newSort.desc !== stateSort.desc
                    ) {
