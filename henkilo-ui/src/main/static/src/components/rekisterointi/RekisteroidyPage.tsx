...
class RekisteroidyPage extends React.Component {
        ...
        this.errorChecks = [
            henkilo =>
                !this.etunimetContainsKutsumanimi(henkilo)
                    ? props.L["REKISTEROIDY_ERROR_KUTSUMANIMI"]
                    : null,
            henkilo =>
                !this.kayttajanimiIsNotEmpty(henkilo)
                    ? props.L["REKISTEROIDY_ERROR_KAYTTAJANIMI"]
                    : null,
            henkilo =>
                !this.passwordsAreSame(henkilo)
                    ? props.L["REKISTEROIDY_ERROR_PASSWORD_MATCH"]
                    : null,
            henkilo =>
                !isValidPassword(henkilo.password)
                    ? props.L["REKISTEROIDY_ERROR_PASSWORD_INVALID"]
                    : null,
            henkilo =>
                !this.kielikoodiIsNotEmpty(henkilo)
                    ? props.L["REKISTEROIDY_ERROR_KIELIKOODI"]
                    : null,
        ]
    ...
    render() {
        return (
            <div
                className="borderless-colored-wrapper rekisteroidy-page"
                style={{marginTop: "50px"}}
            >
                <div className="header-borderless">
                    <p className="oph-h2 oph-bold">
                        {this.props.L["REKISTEROIDY_OTSIKKO"]}
                    </p>
                </div>
                ...
                <div className="flex-horizontal">
                    <div className="wrapper flex-item-1">
                        <RekisteroidyPerustiedot
                            ...
                            updatePayloadModel={this.updatePayloadModelInput.bind(
                                this,
                            )}
                            isLanguageError={
                                !this.kielikoodiIsNotEmpty(this.state.henkilo)
                            }
                            isPasswordError={this.isPasswordError()}
                            isUsernameError={
                                !this.kayttajanimiIsNotEmpty(this.state.henkilo)
                            }
                            isKutsumanimiError={
                                !this.etunimetContainsKutsumanimi(
                                    this.state.henkilo,
                                )
                            }
                        />
                    ...
                    <div className="borderless-colored-wrapper flex-horizontal flex-align-center">
                        <span className="oph-h3 oph-bold">
                            {this.props.L["REKISTEROIDY_VALITSE"]}
                        </span>
                    </div>
                    <div className="wrapper flex-item-1">
                        <RekisteroidyHaka
                            ...
                            temporaryKutsuToken={
                                this.props.kutsu.temporaryToken
                            }
                        />
                    </div>
                </div>
    ...
    updatePayloadModelInput(event) {
        const henkilo =
            StaticUtils.updateFieldByDotAnnotation(
                {...this.state.henkilo},
                event,
            ) || this.state.henkilo
        this.setState({
    ...
    createHenkilo() {
        ...
        this.props.createHenkiloByToken(
            this.props.kutsu.temporaryToken,
            payload,
        )
    }
    ...
    etunimetContainsKutsumanimi(henkilo) {
        return henkilo.etunimet
            .split(" ")
            .filter(nimi => nimi === henkilo.kutsumanimi).length
    }
