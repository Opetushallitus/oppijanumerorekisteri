...
class PalveluCreateContainer extends React.Component<Props, State> {
            ...
            <div className="wrapper">
                {this.state && this.state.error ? (
                    <WideRedNotification
                        ...
                        closeAction={() => this.setError()}
                    />
                ) : null}
                <span className="oph-h2 oph-bold">
                    {this.props.L["PALVELUKAYTTAJAN_LUONTI_OTSIKKO"]}
                </span>
                <PalveluCreateForm
    ...
    onSubmit = async (
        palvelukayttajaCreate: PalvelukayttajaCreate,
    ): Promise<void> => {
        try {
            const palvelukayttajaRead = await this.createPalvelukayttaja(
                palvelukayttajaCreate,
            )
    ...
    createPalvelukayttaja = async (
        palvelukayttaja: PalvelukayttajaCreate,
    ): Promise<PalvelukayttajaRead> => {
