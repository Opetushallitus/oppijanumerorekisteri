...
class OppijaCreateForm extends React.Component<OppijaCreateFormProps, State> {
            ...
            <form onSubmit={this.tallenna}>
                <div className="oph-field oph-field-is-required">
                    ...
                    <input
                        className={classNames("oph-input", {
                            "oph-input-has-error": this.isSubmittedAndHasError(
                                "etunimet",
                            ),
                        })}
                ...
                <div className="oph-field">
                    ...
                    {this.state.submitted && this.state.errors.length > 0 && (
                        <span className="oph-field-text oph-error">
                            {this.props.L["LOMAKE_SISALTAA_VIRHEITA"]}
                        </span>
                    )}
                </div>
            </form>
    ...
    onHenkiloChange = (event: {name: string; value: any}) => {
        ...
        if (this.state.submitted) {
            ...
            if (
                !this.hasError("kutsumanimi") &&
                !isValidKutsumanimi(henkilo.etunimet, henkilo.kutsumanimi)
            ) {
                errors.push({
                    name: "kutsumanimi",
                ...
                state.errors = errors.filter(
                    error => error.name !== "kutsumanimi",
                )
            }
        }
        this.setState(state)
    ...
    isSubmittedAndHasError = (name: string): boolean => {
        return (
            ...
            this.state.errors.findIndex(error => error.name === name) !== -1
        )
    }
    ...
    tallenna = async (event: React.SyntheticEvent<HTMLButtonElement>) => {
    ...
    getHenkilo = (): HenkiloCreate => {
        ...
        return {
            ...this.state.henkilo,
            passinumerot: this.state.form.passinumero
                ? [this.state.form.passinumero]
                : null,
                ...
                ? [
                      {
                          ...
                          ryhmaAlkuperaTieto:
                              properties.YHTEYSTIETO_ALKUPERA_VIRKAILIJA_UI,
                          yhteystieto: [
                              {
