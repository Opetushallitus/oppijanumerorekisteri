...
const TopNavigation = ({
    ...
    const organisaatioList =
        isNoAuthenticationPage || !Array.isArray(organisaatiot)
            ? []
            : organisaatiot
    const roolit: Array<string> = parsePalveluRoolit(organisaatioList)
    const naviTabs =
        route.getNaviTabs &&
        route.getNaviTabs(params && params["oid"], henkilo, route.henkiloType)
    return (
        <div
            id="topNavigation"
            className={classNames({"oph-bg-blue": !isNoAuthenticationPage})}
        >
            {/* Virkailija-raamit looks bad in dev mode because styles are in wrong path. */}
            {!isNoAuthenticationPage && (
                <Script url={urls.url("virkailija-raamit.raamit.js")} />
            )}
            {!isNoAuthenticationPage && (
                <ul className="tabs">
                        ...
                        naviTabs
                            .filter(
                                data =>
                                    isRekisterinpitaja ||
                                    !data.sallitutRoolit ||
                                    data.sallitutRoolit.some(sallittuRooli =>
                                        roolit.includes(sallittuRooli),
                                    ),
                            )
                            .map((data, index) => {
            ...
            {isNoAuthenticationPage && (
                <div>
                    <img src={ophLogo} alt="oph logo" id="ophlogo" />{" "}
                    <img src={okmLogo} alt="okm logo" id="okmlogo" />
                </div>
            )}
        </div>
    )
