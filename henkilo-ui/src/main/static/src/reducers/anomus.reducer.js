...
export type haetutKayttooikeusryhmatState = {|
    +isLoading: boolean,
    +data: Array<HaettuKayttooikeusryhma>
|}

export const haetutKayttooikeusryhmat = (state: haetutKayttooikeusryhmatState = {isLoading: true, data: []}, action: any): haetutKayttooikeusryhmatState => {
    switch (action.type) {
        ...
        case FETCH_HAETUT_KAYTTOOIKEUSRYHMAT_SUCCESS:
            const uudet = action.haetutKayttooikeusryhmat
                .filter( (uusi: HaettuKayttooikeusryhma) => state.data.every( (vanha: HaettuKayttooikeusryhma) => vanha.id !== uusi.id));
        ...
        case CLEAR_HAETTU_KAYTTOOIKEUSRYHMA:
            return {...state, data: state.data.filter( (haettuKayttooikeusryhma: HaettuKayttooikeusryhma) => haettuKayttooikeusryhma.id !== action.id)};
        default:
            return state;
    }
};
