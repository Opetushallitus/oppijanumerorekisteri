...
ssm_target="$( aws ec2 describe-instances --filters 'Name=tag:Name,Values=OppijanumerorekisteriBastion' 'Name=instance-state-name,Values=running' --query 'Reservations[0].Instances[0].InstanceId' --output text )"
