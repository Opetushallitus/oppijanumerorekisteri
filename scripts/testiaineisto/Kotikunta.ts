...
//
//   const kotikunta = Convert.toKotikunta(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

export interface Kotikunta {
    koodiUri:         string;
    resourceUri:      string;
    version:          number;
    versio:           number;
...
export class Convert {
    public static toKotikunta(json: string): Kotikunta[] {
        return cast(JSON.parse(json), a(r("Kotikunta")));
    }

    public static kotikuntaToJson(value: Kotikunta[]): string {
        return JSON.stringify(uncast(value, a(r("Kotikunta"))), null, 2);
    }
}
