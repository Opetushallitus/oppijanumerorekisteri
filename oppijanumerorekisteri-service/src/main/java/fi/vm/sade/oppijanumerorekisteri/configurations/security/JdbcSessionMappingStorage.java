...
public class JdbcSessionMappingStorage implements OphSessionMappingStorage {
    ...
    public JdbcSessionMappingStorage(JdbcTemplate jdbcTemplate, SessionRepository<? extends Session> sessionRepository) {
    ...
    @Override
    public void clean() {
        String sql = "DELETE FROM cas_client_session WHERE session_id NOT IN (SELECT session_id FROM spring_session)";
    ...
    private static class HttpSessionAdapter implements HttpSession {
        ...
        public HttpSessionAdapter(SessionRepository<? extends Session> sessionRepository, Session session) {
            this.sessionRepository = sessionRepository;
