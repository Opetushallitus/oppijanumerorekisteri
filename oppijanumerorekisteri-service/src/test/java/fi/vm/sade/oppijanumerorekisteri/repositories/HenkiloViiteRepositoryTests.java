...
public class HenkiloViiteRepositoryTests extends AbstractRepositoryTest {
    ...
    public void findHenkiloViitteesByHenkilo() {
        ...
        // Search by root:
        List<HenkiloViiteDto> results = this.henkiloViiteRepository.findBy(Set.of(("1.2.3.4.0")));
        ...
        results = this.henkiloViiteRepository.findBy(henkiloOids);
        ...
        // Search by child:
        results = this.henkiloViiteRepository.findBy(Set.of("1.2.3.4.1"));
        assertThat(results).hasSize(2);
        assertThat(results.stream().map(HenkiloViiteDto::getHenkiloOid).collect(toSet()))
                .contains("1.2.3.4.1", "1.2.3.4.2");
        assertThat(results.stream().map(HenkiloViiteDto::getMasterOid).collect(toSet()))
        ...
        results = this.henkiloViiteRepository.findBy(Set.of());
        ...
        results = this.henkiloViiteRepository.findBy(Set.of("1.2.3.4.2"));
    ...
    @Test
    public void largeSetsWorksWithBadCharacters() {
        // Search large queryset (80+)
        Set<String> henkiloOids = new HashSet<>();
        for(int i=1; i<100; i++) {
            henkiloOids.add("1.2.3.4.df" + i);
        }
        List<HenkiloViiteDto> results =this.henkiloViiteRepository.findBy(henkiloOids);
